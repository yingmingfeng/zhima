...
import { registerIpcMain } from '@zhima/electron-ipc/main';
...
import { getDshRunMode } from './dsh/state';
import { showSplashWindow } from './window/splash';
...
const initializeApp = async () => {
  ...
  // 启动优化：内置模式先显示 splash 等待窗口、后台自动启动 DSH CLI；
  // 主窗口暂不显示（showInBackground），DSH 就绪后关 splash 直接露 DSH 窗口，
  // 失败则回退显示主窗口。external 模式维持原行为（主窗口直接显示）。
  const dshAutoStart = getDshRunMode() === 'builtin';
  if (dshAutoStart) showSplashWindow();
  let mainWindow = createMainWindow({ showInBackground: dshAutoStart });

  if (dshAutoStart) {
    // 不 await：CLI 启动耗时不应阻塞 initializeApp 的其余初始化（IPC/托盘/更新等）
    void autoOpenDshOnStartup();
  }
