  ...
  async make({
    ...
    for (const file of output) {
      if (file.endsWith('.exe')) {
        ...
        // TS 5.9 的 lib 将 Uint8Array 泛型默认 ArrayBuffer，与 @types/node 20 的
        // Buffer(extends Uint8Array<ArrayBufferLike>) 推断冲突；此处为 NSIS 二进制
        // 查找标记，运行时一致，用 unknown 断言消除类型歧义。
        const sigOffset = buf.indexOf(sig as unknown as Uint8Array);
        if (sigOffset > 0) {
          ...
          fs.writeFileSync(file, buf as unknown as Uint8Array<ArrayBufferLike>);
