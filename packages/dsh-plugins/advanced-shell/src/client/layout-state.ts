...
export function computeDesktopColumns(
  ...
  const sidebarWidth =
    sidebar === 0 ? collapsedWidth : clamp(sidebar, SIDEBAR_MIN, SIDEBAR_MAX);
  const preferredDetails =
    details === 0 ? 0 : clamp(details, DETAILS_MIN, DETAILS_MAX);
...
export class DesktopLayoutState {
  ...
  setDetails(width: number): void {
    this.publish({
      ...this.snapshot,
      details: clamp(width, DETAILS_MIN, DETAILS_MAX),
    });
  }
