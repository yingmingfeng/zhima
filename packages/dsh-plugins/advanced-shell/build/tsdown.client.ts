...
function browserSourcePath(source: string, sourcemapPath: string): string {
  ...
  const repositoryPath = relative(REPOSITORY_ROOT, physicalSource)
    .split(sep)
    .join('/');
...
export function clientBundle(
  ...
  return ({ env }) => {
    ...
    if (face === 'host')
      return options.hostPhase === true ? node : [SKIP_WORKSPACE_BUILD];
    if (face === 'client')
    ...
    plugins: [
      {
        ...
        resolveId(source: string) {
          const match = /^@deepseek-ai\/dsh-host-apiproxy\/api(?:\/.*)?$/.exec(
            source,
          );
    ...
    plugins: [
      {
        ...
        resolveId(source: string) {
          ...
          if (INLINE_SAFE.test(source) || GENERATED_REMOTE.test(source))
            return null; // wire contribution: inline is the point
          throw new Error(
      ...
      },
      {
        ...
        async load(virtualId: string) {
          ...
          const sourceId = virtualId.slice(
