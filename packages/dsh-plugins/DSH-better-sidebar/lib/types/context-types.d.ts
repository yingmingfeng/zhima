...
export interface SidebarWebUpgradeRoute {
  ...
  handler: (
    req: SidebarHttpRequest,
    socket: SidebarUpgradeSocket,
    head: SidebarUpgradeHead,
  ) => void | Promise<void>;
}
...
export interface SidebarConnectionHandle {
  api: {
    ...
    subagents: {
      history(
        payload: SidebarSubagentAddress & {
          beforeSeq?: number;
          maxMessages?: number;
        },
        signal?: AbortSignal,
...
declare module 'cordis' {
  interface Context {
    ...
    on(
      event: string,
      listener: (session: unknown, event: SidebarSessionEvent) => void,
    ): () => void;
    /**
