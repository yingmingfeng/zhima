...
function bundledNodePath() {
  if (!app.isPackaged) {
    return resolve(process.cwd(), 'resources', 'bin', 'node.exe');
  }
...
function adaptArgv(argv) {
  ...
  if (
    ...
    !runner.includes('dsh-sandbox-windows-acl')
  ) {
    return argv;
  }
  return [bundledNodePath(), aclRunnerPath(), ...rest];
}

/** 沙箱 pwsh shell 提供者（cordis ctx.shell）。 */
export class ZhimaWindowsPwshSandbox extends SandboxPwshExecutor {
  /** @inheritdoc */
...
export default ZhimaWindowsPwshSandbox;